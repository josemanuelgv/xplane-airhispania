...
void MultiplayerEngine::eatThis(const FSD::Message &packet)
{
	...
	if(packet.type == _FSD_CUSTOMPILOT_) // recepción de código ICAO de una aeronave conectada a la red de AHS o de petición de código ICAO propio
	{	

		if(packet.tokens[0] == _FSD_CUSTOMPILOT_PLANEINFO_) // recepción de código ICAO de una aeronave conectada a la red de AHS
		{
			string icao, airline, livery;
			if(!splitMTL(packet.tokens[4], icao, airline, livery))
				return;							// returns false if splitting fails
			...
			switch (t_motor) {
				...
				case '3':
					t_aero = 'H';
					n_motores = '1'; // FIXME: No creo que se pueda sacar de ninguna parte, así que se asume 1
					t_motor = 'T'; // FIXME: No creo que se pueda sacar de ninguna parte, así que se asume 'T'
					break;
				case '5':
					t_aero = 'L';
			...
			if (t_aero == 'H') pilot->cat = 'V'; // Si es helicóptero, cambia la categoría de estela turbulenta a "V" (vertical)
			else pilot->cat = packet.tokens[1];
			pilot->mtlRequested = true;		// just that we dont ask again for the MTL code
			...
			if(pilot->XPregistered) {		// we already know this guy -> send the update to X-Plane
				...
				xivap.addText(colCyan, "MP: Actualizada informacion de Piloto en X-Plane: " + callsign 
					+ " (" + icao + ":" + airline + ":" + livery + ")", true, true);
			}
...
//			pilot->mtl = "A320";
			pilot->mtl = "C182"; // Cambiado a Cessna 182 por defecto (hay más Cessnas 182 en AHS que Airbus 320)
		...
		xivap.addText(colCyan, "MP: Registrando Piloto en X-Plane: " + callsign 
			+ " (" + icao + ":" + airline + ":" + livery + ")", true, true);
	}
...
// FIXME: Quitada petición de parámetros (se podría añadir comando especial para intercambiar "plane params" entre clientes X-Ivap AHS)
...
//			return;
		...
		// Añadida corrección de elevación para intentar que no aparezcan flotando o hundidos los aviones de otros simuladores con otros escenarios
		// Si la velocidad es menor a 40 KIAS, entiendo que el avión está en el suelo (¿habría alguna forma mejor de detectar el "on ground" para un piloto de la red AHS?) 
		if (atof(packet.tokens[5]) < 40.0f && xivap.onGround()) // Si la velocidad del avión de la red es menor a 40 KIAS y nosotros estamos también en tierra
...
//			MultiplayerPilot *pilot;
//			if(pilotIter != _planes.end())  // Busco el avión en la dB para ver de qué categoría es (básicamente si es grande o pequeño) -> FIXME: Buscar otra forma de saberlo
//			{
...
//			}
		...
		pos0->timestamp = xivap.watch.getTime();
		pos0->onground = pilot->onground;
		pos0->p_timestamp = 0;
		pos0->latency = 0;
		
		pilot->interpolator.takeNewPosition(pos0, pilot->distance);

		pilot->CalcDistance(xivap.GetLat(), xivap.GetLon());

		double altdiff = pos0->pos.elevation - xivap.elevationft();
