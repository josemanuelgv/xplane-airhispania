...
public:
	...
	FSD::Message receive();
	FSD::Message testreceive(string str); // Añadido para comando "TEST"
...
//	bool setParams(FSD::PlaneParams p);
	...
	string getMETAR(string estacion); // Añadido para poder obtener directamente un METAR desde la red de AHS
	void subscribeWeather();
	void unsubscribeWeather();
//	void sendPlaneInfo(string mtl);
	void sendPlaneInfo(string mtl, string dcallsign); // Añadido piloto destino al que mandar la información de MTL a través de la red de AHS
