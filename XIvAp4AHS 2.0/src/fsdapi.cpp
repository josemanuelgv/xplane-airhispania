...
/*Conecta al piloto identificado con los parametros*/
...
//					sendInfoRequestReply(m.source, "RN", _realname);
			...
			case _FSD_CUSTOMPILOT_: // Añadido para la red de AHS comando "#SB" con parámetros "PI" (recepción de plane info) y "FSIP2P" (probablemente recepción de plane info exclusiva de FSINN)
				//TODO: Descubrir significado de los tres primeros parámetros (tokens[1]="X", tokens[2]="0" y tokens[3]="0")
				if (m.tokens[0] == _FSD_CUSTOMPILOT_PLANEINFO_) // Comando "PI" (recepción de plane info)
				{
				...
				else if(m.tokens[0] == _FSD_CUSTOMPILOT_REQPLANEINFO_) // recepción de petición código ICAO propio desde una aeronave conectada a la red de AHS
					{
						sendPlaneInfo(xivap.acType(), m.source); // Envía código ICAO (MTL) al piloto (cliente) que lo ha solicitado
					}
...
FSD::Message FsdAPI::testreceive(string str) // Añadido para comando "TEST"
{
	FSD::Message m;
...
//		xivap.addText(colLightGray, line);
		if (xivap.debug.net)
			xivap.addText(colLightGray, line, true, true); // También a log.txt (¡¡CUIDADO: Se grabará también la contraseña!!)
//#endif
		return true;
	}
	return false;
...
//	m.tokens.push_back(fpl.speedtype + fpl.cruisespeed); // El AhsRadar no reconoce el tipo de velocidad, sólo el número
	...
	if (fpl.alttype == "VFR") m.tokens.push_back(fpl.alttype); // Si el tipo de altitud es "VFR", no enviar el dato numérico de altitud
	else m.tokens.push_back(fpl.alttype + fpl.cruisealt); // Si es "A" o "N", enviarlo junto con el dato numérico de altitud
...
string FsdAPI::getMETAR(string estacion)
{
	...
	while (!(m.type == _FSD_WEADATA_RC_AHS_ && m.tokens[0] == _FSD_WEADATA_METAR_AHS_) && !(m.type == _FSD_ERROR_ && m.tokens[0] == "009") && m.source != callsign() && temporiz < temporiz2 + 2) // Espera a que haya mensaje de METAR o error hasta dos segundos (para no paralizar demasiado) FIXME: ¿Se podría hacer de otra forma?
	{ 
...
//	if(send(m)) {
		...
		_usingWX = true;
//	}
	// Añadido para hacer petición de METARs de ADs del FP por adelantado, para meterlos en la dB de meteorología con sus METARs
...
//	m.type = _FSD_REQPLANEINFO_;
	m.type = _FSD_CUSTOMPILOT_; // Incorporado comando de la red AHS para pedir información ("#SB:<callsign_src>:<callsign_dst>:PIR")
	m.source = _callsign;
	m.dest = callsign;
