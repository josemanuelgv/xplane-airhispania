...
//	string dummy1 = "C172";
	...
	_plane_resolution = 5;
//	_default_icao = "A320";
	_default_icao = "C182"; //por defecto se establece una C182
...
/*Devuelve el puntero a la ventana de conexion si no ha sido creada primero (Singleton) y devuelve un puntero a la misma*/
...
/*Devuelve el puntero a la ventana de desconexion si no ha sido creada primero (Singleton) y devuelve un puntero a la misma*/
...
/*Devuelve el puntero a la ventana de mensajes si no ha sido creada primero (Singleton) y devuelve un puntero a la misma*/
...
/*Devuelve el puntero a la ventana de inspeccion de DataReferences si no ha sido creada primero (Singleton) y devuelve un puntero a la misma*/
...
/*Devuelve el puntero a la ventana de formulario para el plan de vuelo si no ha sido creada primero (Singleton) y devuelve un puntero a la misma*/
...
/*Devuelve el puntero a la ventana de formulario para el FMC si no ha sido creada primero (Singleton) y devuelve un puntero a la misma*/
...
/*Establece conexion con el servidor pasando los parametros necesarios*/
...
/*Devuelve el nivel de vuelo en pies*/
...
/*Enviar la actulizacion de la posicion del avion a la red AHS*/
...
/*Obtiene los parametros del avion*/
...
/*Actualiza los parametros del avion desde Xplane*/
...
/*Enviar los parametros del avion*/
...
/*Actualizar la posicion del avion leyendo los DataRef de Xplane*/
void Xivap::updatePosition(PlanePosition* position)
{
	...
	if (ghost) 
	{
		test("PPOS"); // Envía posición de avión fantasma si está activado
	}
}

/*Actualizar el estatus del avion*/
void Xivap::updateStatus()
{
	...
	if(xpdrId != 0) 
	{
		xpdrMode = IdentID; // if identing -> ident
	}
...
/*Devuelve si el piloto automatico esta activado/desactivado*/
...
/*Devuelve el estado del piloto automatico*/
...
/*Sintoniza la radio que se indique en la frecuencia especificada*/
void Xivap::tuneCom(int radio, int freq, string name)
{
	...
	else if (freqStr != UNICOM_FREQ)
	{
		if(debug.teamspeak)
		{
			uiWindow.addMessage(colRed, "Teamspeak: canal no encontrado en AhsControl, buscando en nombres de dependencias conectadas a la red...", true, true);
		}
		name = freq2name(freq); // Movido después de la búsqueda en la página de AHS, para que busque si falla la primera
		if(debug.teamspeak)
		{
			if (pos('.',name) == -1) 
			{
				uiWindow.addMessage(colCyan, "Teamspeak: estacion mas cercana con frecuencia " + freqStr + " MHz : " +  name, true, true);	
			}
			else 
			{
				uiWindow.addMessage(colRed, "Teamspeak: no encontrada estacion cercana conectada para la frecuencia " + freqStr + " MHz", true, true);
			}
		}
	}
...
//			std::vector<string> names_TS = <rutina_que_devuelve_el_nombre_del_canal_TS>
			...
			if (ahsFound) 
			{
				uiWindow.addMessage(colCyan, "Teamspeak: Sintonizando canal '" + name + "'", true, true);
			}
			else if (found) 
			{
				uiWindow.addMessage(colCyan, "Teamspeak: Sintonizando canal '" + p.callsign + "'", true, true);
			}
...
/*Comprueba que la frecuencia sintonizada en ATIS*/
...
/*Devuelve el numero de frecuencia COM con el que se ha sintonizado*/
...
/*Transforma la frecuencia en una cadena de texto*/
string Xivap::freq2str(int freq)
{
	...
	del(s, 0, 3);
    result += s;					// 124.40	
	if ( ( lastdigit == "2" || lastdigit == "7" ) && ls==5 )
	{
		result += "5";
	}
...
/*Devuelve la frecuencia con un nombre correspondiente si tiene*/
string Xivap::freq2name(int freq)
{
	if(freq == UNICOM_FREQ_NUM) 
	{
		return UNICOM_NAME;
	}
	...
	AtcPosition p = _atcList.findFreq(f, lat, lon);
		...
		{
			return f;          // if nothing matches, just return the frequency
		}
}

/*Obtiene la frecuencia de radio activa*/
...
/*Devuelve el numero de canal sintonizado para la frecuencia indicada. 3 si UNICOM . 0 si no hay canal*/
int Xivap::listeningToRadioFreq(string freq)
{
	...
	string f1 = freq2str(_com1freq); 
	while(length(f1) < 7) 
	{
		f1 += "0";
	}
	string f2 = freq2str(_com2freq); 
	while(length(f2) < 7) 
	{
		f2 += "0";
	}
...
/*Gestiona el mensaje indicado y lo acompaña segun el tipo por un color y sonido*/
void Xivap::handleTextMessage(const FSD::Message& m)
{
	float *color = colWhite;
	string message = m.tokens[0];

	if(m.source == _FSD_SERVERDEST_) 
	{
		color = colLightBlue; // server message
	}
	else {
		// radio chatter
		if(m.dest[0] == '@') {
			...
			if(com > 0) {
				...
				// radio message to me
				if(copy(message, 0, length(fsd.callsign())) == fsd.callsign()) 
				{
					to_me = true;
				}
				if(com == _activeRadio) {
...
/*Gestiona la informacion de respuesta*/
...
/*Establece la radio como acoplada*/
...
void Xivap::handleError(const FSD::Message &m)
{
	...
	if(m.tokens[0] != "009") 
	{
		uiWindow.addMessage(colRed, errstr, true, true); // No mostrar en ventana el error 009 "No such weather profile"
	}
	else if (debug.weather > 2) 
	{
		uiWindow.addMessage(colRed, errstr, true, true); // FIXME: ¿Sería posible sólo mostrarlo en log y no en ninguna ventana sin causar deterioro de rendimiento?
	}
}

/*Gestiona un mensaje de baneo de la red mostrando el motivo si lo hay*/
...
/*Desconectar el plugin de la red*/
...
//Macro para acumular mensajes en la variable changes separados por comas
#define HANDLECHG(message) { changed = true; if(length(changes) > 0) changes += ", "; changes += message; }

/*Activa el modo depuracion con el comando indicado. */
...
//FIXME: DEBUG
	...
	else if (mlinea.dest == "PPOS") // Mandar posición de presunto piloto de la red, para visualizar un avión
	{
		...
		{
			m.tokens.push_back(itostring((int)(elevationft() + atof(mlinea.tokens[0])))); // 1er. parámetro: altitud extra añadida para aviones grandes
		}
		else
		{
...
//		m.tokens.push_back("4290774782"); // PBH de avión cogido al azar de una traza de la red
	...
	//	else m.dest = "AHS120D";
		...
		if (mlinea.tokens.size() > 2) 
		{
			m.tokens.push_back(mlinea.tokens[3]); // 3er. parámetro: el segundo número del comando (de momento puede ser 0 ó 1)
		}
		else
		{
			m.tokens.push_back("0");
		...
		if (mlinea.tokens.size() > 0) 
		{
			m.tokens.push_back("~" + mlinea.tokens[0]); // 1er. parámetro: ICAO de la aeronave
		}
		else 
		{
			m.tokens.push_back("~" + _default_icao);
		}
		if (mlinea.tokens.size()> 1) 
		{
			altgrande = atof(mlinea.tokens[1]); // 2º parámetro: Altitud en tierra extra añadida para aviones grandes (TODO: Hacerlo configurable, por cada MTL si fuera posible)
		}
		string ac0 = m.tokens[4]; // ICAO de aeronave
		if (ac0[0] == '~') 
		{
			del(ac0,0,1); // Borra el primer carácter "~" que suele mandar la red (no sé si significará algo)
		}
		m.tokens[4] = ac0;
	}
...
//		else m.dest = "AHS120D";
	...
	else if (mlinea.dest == "GHOST")
	{
		...
		if (debug.multiplayer)
		{
			if (ghost)
			{
				addText(colYellow, "'Aeronave fantasma' activada", true, true);
			}
			else 
			{
					addText(colRed, "'Aeronave fantasma' desactivada", true, true);
			}
		}
...
/*Getiona cambios en el plan de vuelo*/
void Xivap::handleFPchange(const FSD::Message& m)
{
	...
	if (pos("F",new_fp.cruisealt)==0)	
	{
		del(new_fp.cruisealt, 0, 1);
		new_fp.alttype="F";
	}
	else if (pos("A",new_fp.cruisealt)==0)	
	{
		del(new_fp.cruisealt, 0, 1);
		new_fp.alttype="A";
	}
	else if (pos("VFR",new_fp.cruisealt)==0)	
	{
		del(new_fp.cruisealt, 0, 3);
		new_fp.alttype="VFR";
	}

	if(fpl.cruisespeed != new_fp.cruisespeed) 
	{ 
		HANDLECHG("cruise speed"); 
		fpl.cruisespeed = new_fp.cruisespeed; 
	}

	if(fpl.cruisealt != new_fp.cruisealt) 
	{
		HANDLECHG("cruise altitude"); 
		fpl.cruisealt = new_fp.cruisealt; 
	}

	if(fpl.deptimeact != new_fp.deptimeact) 
	{ 
		HANDLECHG("dep. time"); 
		fpl.deptimeact = new_fp.deptimeact; 
	}

	if(fpl.departure != new_fp.departure) 
	{ 
		HANDLECHG("dep. airport"); 
		fpl.departure = new_fp.departure; 
	}

	if(fpl.deptimeest != new_fp.deptimeest) 
	{ 
		HANDLECHG("est. dep. time"); 
		fpl.deptimeest = new_fp.deptimeest; 
	}

	if(fpl.destination != new_fp.destination) 
	{ 
		HANDLECHG("destination"); 
		fpl.destination = new_fp.destination; 
	}

	if(fpl.hoursenroute != new_fp.hoursenroute) 
	{ 
		HANDLECHG("hours enroute");
		fpl.hoursenroute = new_fp.hoursenroute; 
	}

	if(fpl.minenroute != new_fp.minenroute)
	{ 
		HANDLECHG("minutes enroute"); 
		fpl.minenroute = new_fp.minenroute; 
	}

	if(fpl.alternate != new_fp.alternate) 
	{ 
		HANDLECHG("alternate"); 
		fpl.alternate = new_fp.alternate;
	}

	if(fpl.remarks != new_fp.remarks) 
	{ 
		HANDLECHG("remarks"); 
		fpl.remarks = new_fp.remarks; 
	}
	if(fpl.route != new_fp.route) 
	{ 
		HANDLECHG("route"); 
		fpl.route =new_fp.route ; 
	}
	if(fpl.alttype!=new_fp.alttype) 
	{
		HANDLECHG("Alttype"); 
		fpl.alttype =new_fp.alttype ; 
	}
...
/*Sintoniza el canal de radio 1-2 indicado*/
...
/*Gestiona los mensajes de respuesta del ATIS*/
...
//			del(channel, 0, p+1);
		...
		if(copy(com1name,3,1) == ".")
		{
			com1name=freq2name(_com1freq);
		}
		if(copy(com2name,3,1) == ".") 
		{
			com2name=freq2name(_com2freq);
		}
		if(_activeRadio==1) {
...
/* Procesar una linea con un comando*/
...
/*Cambia el avion y toma el modelo de avion actualmente en uso*/
...
void Xivap::handleCommand(string line)
{
	...
	if(command == "R") {
		...
		if(p == -1) 
		{
			return;		// callsign ends at the first space
		}
...
*/
		...
		if (caja.Enabled()) // Sólo hace algo si la caja está habilitada
		{
			if (caja.Activada()) 
			{
				caja.Fin(); // Finaliza la caja negra en caso de que esté activada
			}
...
//		return;
		...
		if(!((freq >= 11800 && freq <= 13697) || (freq >= 118000 && freq <= 136970))) 
		{
			return; // 6 digit check
		}
		if(com == 1) {
			// com1
			if(freq != _com1freq) 
	...
	{ // Añadido comando para cambiar de canal directamente en el Teamspeak
		line = trim(strupcase(line));
		tsRemote.SwitchChannel("AHS"+fsd.vid(), fsd.password(), string(AHS_SERVER_URL), fsd.callsign(), line);
		return;

	}	else if (command=="ICAO") 
	{ // Añadido comando para cambiar el ICAO de la aeronave propia para mostrar en red
		line = trim(strupcase(line));
	...
	{ // Añadido comando para aplicar niveles de debug al vuelo (parámetros separados por ":")
		line = trim(strupcase(line));
...
/*Establecer el tipo de avion, aerolinea y librea*/
...
//FIXME: DEBUG
		if (debug.weather > 0 && use_glob)
		{
			xivap.addText(colRed, "No se han encontrado estaciones cercanas con datos de meteorologia (distancia a la estacion mas cercana = " + itostring((int)stations[0].distance) + ")", true, true);
		}	
	}
...
//FIXME: DEBUG
	...
	//		}
			if (_fplSend)
			{
				...
				if (ADOrg.metar != "")
				{
					metarOrg = ADOrg.metar;
				}
				else 
				{
					metarOrg = fpl.departure + ": No hay METAR disponible";
				}
				if (ADDest.metar != "") 
				{
					metarDest = ADDest.metar;
				}
				else 
				{
					metarDest = fpl.destination +": No hay METAR disponible";
				}
				if (ADAlt.metar != "")
				{
					metarAlt = ADAlt.metar;
				}
				else 
				{
					metarAlt = fpl.alternate + ": No hay METAR disponible";
				}
				...
				if (infometars == true)
				{
					...
					{
						msgWindow.addMessage(colWhite,"METAR Actual: " + currentWxStation.metar + " (Tiempo CAVOK forzado)\n\n"); 
					}
					...
					if (xivap.fpl.alternate2 != "") // Si existe 2º AD alternativo, mostrar su METAR también
					{
						...
						if (ADAlt2.metar != "") 
						{
							metarAlt2 = ADAlt2.metar;
						}
						else
						{
							metarAlt2 = fpl.alternate2 + ": No hay METAR disponible";
						}
...
/*Activa/desactiva mensajes privados*/
...
/*Activa/desactiva etiquetas de los aviones*/
	...
	{ // turn on MP
		if(!_multiplayer.initialized()) {
			if(!_multiplayer.init()) {
				uiWindow.addMessage(colRed, "Fallo al inicializar el multijugador: " + _multiplayer.errorMessage(), true, true);
				_useMultiplayer = false;
				return;
			}
		}
...
/*Descarga el estado actual de AHSControl*/
