...
//	xivap.addText(colCyan, "Teamspeak: switch to channel " + atccall + " on " + server, true, true);
	xivap.addText(colCyan, "Teamspeak: cambiando a canal " + atccall + " en " + string(AHS_SERVER_URL), true, true);
#ifdef APPLE
    string url = GenerateURL(vid, pass, string(AHS_SERVER_URL), pilotcall, atccall);
  ...
  bool library(TryWithLibrary(vid, pass, string(AHS_SERVER_URL), pilotcall, atccall));
                               ...
                               string& atccall)
{
  ...
  // get server details
	...
	if(err == -1)
		return SdkConnect(vid, pass, string(AHS_SERVER_URL), pilotcall, atccall);
	// another error...
	else if(err != 0)
	{
	...
	if(tsserver != string(AHS_SERVER_URL))
		return SdkConnect(vid, pass, string(AHS_SERVER_URL), pilotcall, atccall);
                           ...
                           string& atccall)
{
  ...
  // try to connect...
	if(tsrConnect((char*)pconst(GenerateURL(vid, pass, string(AHS_SERVER_URL), pilotcall, atccall))) == 0)
		return true;
...
string TeamSpeak::GenerateURL(const string& vid, 
                               ...
                               const string& atccall)
{
	string teamspeak = "teamspeak://" + string(AHS_SERVER_URL)+ "?loginname=" + vid + "?password=" + strupcase(pass) + "?channel=" + atccall + "?nickname=" +  pilotcall + "-" + xivap.realname + "-" + xivap.baseairport;
  //teamspeak.Format("teamspeak://%s?nickname=%s?loginname=%s?password=%s?channel=%s", 
  //                  server, pilotcall, vid, pass, atccall);
	// FIXME: DEBUG
	if (xivap.debug.teamspeak)
		xivap.addText(colCyan, "Teamspeak URL: " + teamspeak, true, true);
  return teamspeak;
}
