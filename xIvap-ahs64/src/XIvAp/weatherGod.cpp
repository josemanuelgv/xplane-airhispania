...
//#else
		...
		{
			WXf(ref_wind_altitude_msl_m[i], wind_altitude_msl_m[i]);
			WXf(ref_wind_direction_degt[i], wind_direction_degt[i]);
			WXf(ref_wind_speed_kt[i], wind_speed_kt[i]);
			WXf(ref_shear_direction_degt[i], shear_direction_degt[i]);
			WXf(ref_shear_speed_kt[i], shear_speed_kt[i]);
			WXf(ref_turbulence[i], turbulence[i]);
		}
...
//#endif
	...
	if (xivap.debug.weather > 2)
	{
		...
		DEB_WXf(string("'Microburst': "), ref_microburst_probability, microburst_probability);	
		DEB_WXf(string("Porcentaje de lluvia: "), ref_rain_percent, rain_percent);
		DEB_WXf(string("'Friccion' de pista: "), ref_runway_friction, rain_percent);
		DEB_WXf(string("Visibilidad reportada: "), ref_visibility_reported_m, visibility_reported_m);
		DEB_WXf(string("Porcentaje de termales: "), ref_thermal_percent, thermal_percent);
		DEB_WXf(string("Ritmo de termales: "), ref_thermal_rate_ms, thermal_rate_ms);
		DEB_WXf(string("Altitud MSL de termales: "), ref_thermal_altitude_msl_m, thermal_altitude_msl_m);
	}
	else
	{
...
void WeatherGod::init()
{
	...
	haveWeather = false;
	initialized = true;
	xivap.cavok = false; // inicializa a false para no tener tiempo CAVOK por defecto
	// Recolecta METARs de los ADs en el plan de vuelo (FIXME: habría que buscar de ADs más próximos en caso de que no hubiera disponible para los ADs seleccionados)
//	if (xivap.fpl.departure != "") xivap.metarOrg = xivap.fsd.sendWxRequest(FSD::WX_METAR, xivap.fpl.departure);
//	if (xivap.fpl.destination != "") xivap.metarDest = xivap.fsd.sendWxRequest(FSD::WX_METAR, xivap.fpl.destination );
//	if (xivap.fpl.alternate != "") xivap.metarAlt = xivap.fsd.sendWxRequest(FSD::WX_METAR, xivap.fpl.alternate);
//	if (xivap.fpl.alternate2 != "") xivap.metarAlt2 = xivap.fsd.sendWxRequest(FSD::WX_METAR, xivap.fpl.alternate2);
...
//		case 1:case 2:case 3: result=10;break; // High Cirrus = 1
		case 1:case 2: result=10;break; // High Cirrus = 1
//		case 4: result=20;break; // Scattered = 2
		case 3: case 4: result=20;break; // Scattered = 2
//		case 5: result=30;break; //Broken = 3
		case 5: case 6: result=30;break; //Broken = 3
//		case 6:case 7: result=40;break; //Overcast = 4
		case 7: case 8: result=40;break; //Overcast = 4
//		case 8:case 9: case 10: result= 50;break; //stratus=5
		case 9: case 10: result= 50;break; //stratus=5
...
//		#endif
		...
		if(s.cloudLayers[i].precip != FSD::PRECIP_NONE) {
			if(s.cloudLayers[i].precipbase < aircraftAlt) {
				if(s.cloudLayers[i].tops > aircraftAlt) {
					rain = true;

//FIXME: DEBUG
					if (xivap.debug.weather > 2)
						xivap.addText(colRed,"rain = true",true,true);
...
//FIXME: DEBUG
					if (xivap.debug.weather > 2)
						xivap.addText(colRed,"rainPercent = " + ftoa(rainPercent),true,true);
					}
				}
			}
		}
...
//FIXME: DEBUG
		if (xivap.debug.weather > 2)
			xivap.addText(colRed, s.name + " NO INICIALIZADO", true, true);
...
//FIXME: DEBUG			
		if (xivap.debug.weather > 2)
			xivap.addText(colRed, s.name + " NO VALIDO (" + itostring((int)s.windLayers.size()) +" " + itostring((int)s.visLayers.size()) + " " + itostring((int)s.tempLayers.size()) + " " + itostring((int)s.lat) + " " + itostring((int)s.lon) + " " + s.name, true, true);
