...
//   if( (unitsPtr = strstr( wind, "KT") ) != NULL )
   ...
   if( charcmp(wind,"'V''R''B'dd'K''T'")) {
	  ...
	  if (strstr(dummy, "MPS") != NULL) Mptr->winData.windSpeed /= 0.5144f;
	  else if (strstr(dummy, "KMH") != NULL) Mptr->winData.windSpeed /= 1.852f;
      memset(Mptr->winData.windUnits, '\0', 4);
...
*/
   ...
   if( charcmp(wind,"'V''R''B'ddd'G'ddd'K''T'")) {
	  ...
	  if (strstr(dummy, "MPS") != NULL)
	  {
		  Mptr->winData.windSpeed /= 0.5144f;
		  Mptr->winData.windGust /= 0.5144f;
	  }
	  else if (strstr(dummy, "KMH") != NULL)
	  {
		 Mptr->winData.windSpeed /= 1.852f;
		 Mptr->winData.windGust /= 1.852f;
	  }
